class Rope
  {
    //Número de enlaces y punto de conexión
  ...
  //Composite.stack crea los múltiples cuerpos rectangulares
  const rects = Composites.stack(100, 100, this.nlink, 1, 5, 5, function(x, y) {
    
    //Te regresa un cuerpo rectangular y te dan sus propiedades
    return Bodies.rectangle(x, y, 30, 5, { collisionFilter: { group: group } });
  });
  
  //Todas las propiedades van declaradas con this
  this.pointA = pointA;

  //Composites.chain generas una cadena enlazando los rectángulos creados
  this.body = Composites.chain(rects, 0.1, 0, -0.6, 0, {stiffness: 0.1, length: 0.1, render: {type: 'line'}});
      
  World.add(engine.world, this.body);
  
    //Composite.add agrega todos los rectángulos a Composite
    //Constraint.create crea estos nuevos rectángulos
